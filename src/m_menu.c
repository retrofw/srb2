...
static void M_DrawControl(void)
{
	...
	M_CentreText (ControlDef.y-12,
		 (setupcontrols_secondaryplayer ? "SET CONTROLS FOR SECONDARY PLAYER" :
		                                  "PRESS START TO CHANGE, R TO CLEAR"));
#else
	M_CentreText (ControlDef.y-12,
		 (setupcontrols_secondaryplayer ? "SET CONTROLS FOR SECONDARY PLAYER" :
...
static void M_DrawVideoMode(void)
{
	...
	{
#ifdef GCW0
		M_CentreText(VidModeDef.y + 60 + 16, "Press START to set mode");
#else
...
#endif
#ifdef GCW0
		M_CentreText(VidModeDef.y + 100 + 16,"Press SELECT to exit");
#else
...
static void M_DrawLoad(void)
{
	...
	M_DrawGenericMenu();
#ifdef GCW0
	V_DrawCenteredString(BASEVIDWIDTH/2, 40, 0, "Hit R to delete a save.");
#else
